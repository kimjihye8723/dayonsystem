...
const DEVICE_PORT = 10001;

const TARGET_HOST = '183.111.206.87';
    ...
    if (cmd === 0x2002 && currentStep === 'version_negotiation') {
        ...
        if (currentStep === 'sdk_negotiation') {
            ...
            if (guidMatch && guidMatch[1]) {
                ...
                console.log(`[STEP 3] SetSDKTcpServer 요청 전송 (Target: ${TARGET_HOST}:${TARGET_PORT})...`);
                currentStep = 'set_tcp_server';
                const setServerXml = `<?xml version="1.0" encoding="utf-8"?><sdk guid="${sessionGuid}"><in method="SetSDKTcpServer"><server host="${TARGET_HOST}" port="${TARGET_PORT}"/></in></sdk>`;
                sendXml(sessionGuid, setServerXml);
            } else {
                console.error("GUID를 찾을 수 없습니다:", xmlData);
                client.destroy();
            }
        } 
        else if (currentStep === 'set_tcp_server') {
            console.log(`\n==================================================`);
            console.log(`[SetSDKTcpServer 결과 수신]`);
            console.log(xmlData);
            console.log(`==================================================\n`);
            
            // 변경이 잘 되었는지 확인하기 위해 GetSDKTcpServer 호출
            console.log(`[STEP 4] 변경 확인용 GetSDKTcpServer 요청 전송...`);
            ...
            console.log(`\n==================================================`);
            console.log(`[GetSDKTcpServer 최종 확인 결과]`);
            console.log(xmlData);
            console.log(`==================================================\n`);
            client.destroy();
        }
